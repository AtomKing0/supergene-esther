...
public class FlexBuffersBuilder {
    ...
    private final ReadWriteBuf f936bb;
    ...
    private int align(int i10) {
        int i11 = 1 << i10;
        int iPaddingBytes = Value.paddingBytes(this.f936bb.writePosition(), i11);
        while (true) {
            int i12 = iPaddingBytes - 1;
            if (iPaddingBytes == 0) {
                return i11;
            }
            this.f936bb.put((byte) 0);
            iPaddingBytes = i12;
        }
    }
    ...
    private Value createKeyVector(int i10, int i11) {
        ...
        while (i12 < this.stack.size()) {
            i12++;
            iMax = Math.max(iMax, Value.elemWidth(4, 0, this.stack.get(i12).key, this.f936bb.writePosition(), i12));
        }
        int iAlign = align(iMax);
        writeInt(j10, iAlign);
        int iWritePosition = this.f936bb.writePosition();
    ...
    private Value createVector(int i10, int i11, int i12, boolean z10, boolean z11, Value value) {
        ...
        int iMax = Math.max(0, widthUInBits(j10));
        if (value != null) {
            iMax = Math.max(iMax, value.elemWidth(this.f936bb.writePosition(), 0));
            i13 = 3;
        } else {
            i13 = 1;
        }
        ...
        for (int i16 = i11; i16 < this.stack.size(); i16++) {
            iMax2 = Math.max(iMax2, this.stack.get(i16).elemWidth(this.f936bb.writePosition(), i16 + i13));
            if (z10 && i16 == i11) {
        ...
        if (!z10) {
            while (i17 < this.stack.size()) {
                this.f936bb.put(this.stack.get(i17).storedPackedType(iMax2));
                i17++;
            }
        }
        if (value != null) {
    ...
    private int putKey(String str) {
        ...
        if ((this.flags & 1) == 0) {
            ...
            this.f936bb.put(bytes, 0, bytes.length);
            this.f936bb.put((byte) 0);
        ...
        this.f936bb.put(bytes2, 0, bytes2.length);
        this.f936bb.put((byte) 0);
        this.keyPool.put(str, Integer.valueOf(iWritePosition));
        return iWritePosition;
    }
    ...
    private Value writeBlob(int i10, byte[] bArr, int i11, boolean z10) {
        ...
        int iWritePosition = this.f936bb.writePosition();
        this.f936bb.put(bArr, 0, bArr.length);
        if (z10) {
            this.f936bb.put((byte) 0);
        }
    ...
    private void writeDouble(double d10, int i10) {
        if (i10 == 4) {
            this.f936bb.putFloat((float) d10);
        } else if (i10 == 8) {
            this.f936bb.putDouble(d10);
        }
    }

    private void writeInt(long j10, int i10) {
        if (i10 == 1) {
            this.f936bb.put((byte) j10);
            return;
        }
        if (i10 == 2) {
            this.f936bb.putShort((short) j10);
        } else if (i10 == 4) {
            this.f936bb.putInt((int) j10);
        } else {
            if (i10 != 8) {
                return;
            }
            this.f936bb.putLong(j10);
        }
    }

    private void writeOffset(long j10, int i10) {
        writeInt((int) (((long) this.f936bb.writePosition()) - j10), i10);
    }
    ...
    public ByteBuffer finish() {
        int iAlign = align(this.stack.get(0).elemWidth(this.f936bb.writePosition(), 0));
        writeAny(this.stack.get(0), iAlign);
        this.f936bb.put(this.stack.get(0).storedPackedType());
        this.f936bb.put((byte) iAlign);
        this.finished = true;
        return ByteBuffer.wrap(this.f936bb.data(), 0, this.f936bb.writePosition());
    }
    ...
    public FlexBuffersBuilder(ReadWriteBuf readWriteBuf, int i10) {
        ...
        this.keyComparator = new Comparator<Value>() { // from class: androidx.emoji2.text.flatbuffer.FlexBuffersBuilder.1
            ...
            public int compare(Value value, Value value2) {
                ...
                int i12 = value2.key;
                do {
                    b10 = FlexBuffersBuilder.this.f936bb.get(i11);
                    b11 = FlexBuffersBuilder.this.f936bb.get(i12);
                    if (b10 == 0) {
                        return b10 - b11;
                    }
                    i11++;
