...
public final class TextRenderer extends BaseRenderer implements Handler.Callback {
    ...
    private void clearOutput() {
        updateOutput(new CueGroup(y.q(), getPresentationTimeUs(this.lastRendererPositionUs)));
    }
    ...
    private void renderFromCuesWithTiming(long j10) {
        ...
        if (andDecodeCuesWithTiming) {
            y<Cue> cuesAtTimeUs = this.cuesResolver.getCuesAtTimeUs(j10);
    ...
    public int supportsFormat(Format format) {
        if (isCuesWithTiming(format) || this.subtitleDecoderFactory.supportsFormat(format)) {
            return k2.c(format.cryptoType == 0 ? 4 : 2);
        }
        return MimeTypes.isText(format.sampleMimeType) ? k2.c(1) : k2.c(0);
    }
