...
//    broadcastEvent(EVENTS.VIEWABLECHANGE, isViewable);
    if( isViewable != _isViewable){
        console.log('Set isViewable to ' + stringify(_isViewable) );
        isViewable = _isViewable;
        broadcastEvent(EVENTS.INFO, 'Set isViewable to ' + stringify(isViewable));
        broadcastEvent(EVENTS.VIEWABLECHANGE, isViewable);
    }
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////
    abm.onEngagementEvent = function(event) {
      var args  = ['onEngagementEvent', 'event', event];
      bridge.executeNativeCall(args);
    };

    abm.onReward = function() {
      var args = ['onReward'];

      bridge.executeNativeCall(args);
    };

    abm.onCtaClick = function(url) {
      var args = ['onCtaClick', 'url', url];

      bridge.executeNativeCall(args);
    };

    abm.onCriticalError = function(error) {
      var args = ['onCriticalError', 'error', error];
