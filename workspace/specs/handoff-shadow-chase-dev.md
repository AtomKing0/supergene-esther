...
**신규 4종은 클라 번들 반영 + 커밋·푸시 완료**(`.meta` 포함). 개발이 할 일은 **프리로드 등록 + 호출부 작성**임.
...
- 번들은 기존 관례대로 **mp3만** 둠(같은 이름의 wav/mp3 공존 시 에셋명 충돌)
...
| ④ | ~~**타임존**~~ | **해소(2026-07-31 기획 확정)** — 플랫폼별 기준이 맞음: **FB=UTC · 토스=KST**. 토스는 한국 전용 서비스라 경계가 9시간 다른 것은 의도된 설계이며, 일 참여 상한이 플랫폼별로 달리 걸리는 것도 수용함. 각 서버는 자기 기준 시각으로 월~금 경계를 자르면 됨 |
