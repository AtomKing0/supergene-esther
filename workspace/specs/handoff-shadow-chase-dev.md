...
>
> **7-28 변경 요약** — ① 팝업 세션 상한 시 발동 연출 **이월**(유실 금지) ② 결과 화면 **RV ×2 유지**(하단 체스트 블록만 추가) ③ **타이머 말풍선 [+5 min]** 연장 추가(**드롭 가능**) ④ 실패 **토스트 1회** 추가 ⑤ 로그 필드 보강 + `7109` 신설 ⑥ 강제 시청 **6초 상한**
...
- [ ] 잔여 시간 ≤ `ad_extend_threshold_sec`(300) AND 미완주 → **행잉 태그의 타이머 옆에 말풍선 [+5 min]** 노출 (꼬리가 타이머를 지시 · 크림 바탕 + 금 테두리 + 틸 TV 아이콘 · 신규 모달 없음)
...
| **7109_SHADOW_CHASE_AD_EXTEND** | **신규**(연장 채택 시) — 타이머 말풍선 [+5 min] `step(impression/watch_start/complete)`·`remain_sec_before`·`extend_sec`·`result` |
...
- [ ] ★**타이머 연장**(채택 시) — 잔여 5분 이하에서만 말풍선 노출 · 사이클당 1회 초과 불가(서버 검증) · 시청 중 강종 시 중복 연장 없음
