...
>
> **7-28 변경** — §3을 spec-027 현행안으로 정합(구안 잔재 제거: 밧줄 레드닷·유저 탭 발동·12:00/19:00 슬롯 → **자동 발동·팝업 체인 마지막 순번**). 실패 토스트는 **미채택으로 확정**되어 삭제. **AS-IS에는 팝업 큐·세션 상한이 존재하지 않으므로**(§0) 이를 전제했던 "발동 연출 이월" 규칙은 **전량 제거**함 — 현행 하드코딩 await 체인에서는 앞 팝업이 많아도 발동 연출이 잘리지 않음.
...
- **예외 — `shadow_activate`**: 레드닷 강등이 불가능한 팝업임(연출 재생 = 타이머 시작). **세션 상한을 도입하더라도 이 팝업은 상한 대상에서 제외**할 것.
...
| 시점 | 노출 | 티어·모드 |
|---|---|---|
| 레벨10 클리어 직후 로비 | **언락 인트로 없음** (unlock 50031 · `show_tutorial=FALSE` · tutorial_guide 미등록) | 큐 진입 없음 |
...
| 해금 직후 발동 조건 충족 | 언락 인트로가 없으므로 **2모달 충돌 없음** — 발동 연출 단독 재생 | — |


- 타 이벤트 공통 원칙: 강제 팝업은 P1(언락 1회)·P2(정산)만, 나머지 전부 레드닷 유도. **본 이벤트의 발동 연출은 예외** — 레드닷으로 강등하면 이벤트가 성립하지 않으므로(자동 발동이 설계 전제), **세션 상한을 도입하더라도 상한 대상에서 제외**할 것.
...
| shadow_activate | P4 | cycle | force | false | **그림자 냥이 추격전 발동 연출** — 체인 최후미·레드닷 강등 불가·**세션 상한 도입 시 상한 대상 제외** |
...
1) cap 값(NRU 1 고정 vs 일반 1~2 A/B) 2) event_milestone force vs reddot A/B(세션당 게임수) 3) NRU 정의(레벨 vs D일 vs AND) 4) idle_gold 강제 vs 큐 5) 레드닷 강등분 만료 정책
