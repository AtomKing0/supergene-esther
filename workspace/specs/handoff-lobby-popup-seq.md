...
>
> **7-28 변경** — §3을 spec-027 현행안으로 정합(구안 잔재 제거: 밧줄 레드닷·유저 탭 발동·12:00/19:00 슬롯 → **자동 발동·팝업 체인 마지막 순번**). **세션 상한 소진 시 이월 규칙** 추가.
...
- **토스트는 모달이 아님** — 상한 계산·큐 대상에서 제외(비차단 노출).
...
| 타이머 만료(실패) | **다음 로비 복귀 시 토스트 1회**(`T_SHADOW_CHASE_FAIL_FLEE`) | **비모달 토스트** — 체인 미점유·상한 계산 제외 |
| 동시 엣지: 언락 인트로 + 발동 조건 | P1 언락 인트로 먼저 → **발동 연출은 다음 로비 복귀로 이월** (같은 진입 2모달 금지) | — |
| ★**세션 상한 소진** | **발동 연출을 잘라내지 말고 다음 로비 복귀로 이월** | **이월 필수 — 아래 참조** |

### ★ 발동 연출 이월 규칙 (필수)

발동 연출은 체인 **마지막** 순번이라 앞 팝업(⓪~⑧)이 세션 상한을 소진하면 큐에서 잘릴 수 있음. 그런데 **연출 재생 = 타이머 시작**이므로, 잘리면 `7102`가 발화되지 않고 **사이클이 통째로 유실**됨.

- 상한 초과 시 **소멸·레드닷 강등이 아니라 보류(pending)** — 다음 로비 복귀에서 재평가
- 보류 중 `cooldown_sec` 타이머 **미시작** · `7101` **재발화 없음**(참여율 분모 중복 계상 방지)
- 이벤트 기간을 벗어나면 보류분 폐기
- `7101.deferred(bool)` / `7102.deferred_count`로 이월 발생량 계측
...
- 실패 토스트 + 다른 팝업 동시 → 토스트는 비모달이라 병행 노출 가능(체인 차단 없음)
