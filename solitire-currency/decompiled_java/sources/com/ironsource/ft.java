package com.ironsource;

import android.annotation.SuppressLint;
import android.os.Handler;
import android.webkit.WebResourceError;
import android.webkit.WebResourceRequest;
import android.webkit.WebSettings;
import android.webkit.WebView;
import android.webkit.WebViewClient;
import androidx.media3.exoplayer.ExoPlayer;
import com.ironsource.ft;
import com.ironsource.mediationsdk.IronSource;
import com.ironsource.mediationsdk.testSuite.TestSuiteActivity;
import com.ironsource.ns;
import java.util.Iterator;
import java.util.List;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;
import org.json.JSONException;
import org.json.JSONObject;

/* JADX INFO: loaded from: classes4.dex */
public final class ft implements ss {

    /* JADX INFO: renamed from: a, reason: collision with root package name */
    @NotNull
    private final Handler f12174a;

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    @Nullable
    private WebView f12175b;

    /* JADX INFO: renamed from: c, reason: collision with root package name */
    @NotNull
    private final WebView f12176c;

    /* JADX INFO: renamed from: d, reason: collision with root package name */
    @NotNull
    private final JSONObject f12177d;

    /* JADX INFO: renamed from: e, reason: collision with root package name */
    private boolean f12178e;

    /* JADX INFO: renamed from: f, reason: collision with root package name */
    private boolean f12179f;

    /* JADX INFO: renamed from: g, reason: collision with root package name */
    @NotNull
    private final ig f12180g;

    public static final class a extends WebViewClient {

        /* JADX INFO: renamed from: b, reason: collision with root package name */
        final /* synthetic */ String f12182b;

        a(String str) {
            this.f12182b = str;
        }

        @Override // android.webkit.WebViewClient
        public void onPageFinished(@Nullable WebView webView, @Nullable String str) throws JSONException {
            if (!ft.this.f12178e && str != null && p9.q.K(str, String.valueOf(this.f12182b), false, 2, null)) {
                ft ftVar = ft.this;
                List<? extends Object> listE = kotlin.collections.u.e(ftVar.f12177d);
                kotlin.jvm.internal.t.g(listE, "null cannot be cast to non-null type kotlin.collections.List<kotlin.Any?>");
                ftVar.a(ftVar.a("onDataReady", listE));
                os.f14268a.d();
                ft.this.f12178e = true;
            }
            super.onPageFinished(webView, str);
        }

        @Override // android.webkit.WebViewClient
        public void onReceivedError(@NotNull WebView view, @NotNull WebResourceRequest request, @NotNull WebResourceError error) throws JSONException {
            kotlin.jvm.internal.t.i(view, "view");
            kotlin.jvm.internal.t.i(request, "request");
            kotlin.jvm.internal.t.i(error, "error");
            os.f14268a.a(error.toString());
            zs.f16492a.b("got error during TestSuite web controller loading " + error);
            super.onReceivedError(view, request, error);
        }
    }

    public static final class b implements qg {

        /* JADX INFO: renamed from: b, reason: collision with root package name */
        final /* synthetic */ qg f12184b;

        b(qg qgVar) {
            this.f12184b = qgVar;
        }

        /* JADX INFO: Access modifiers changed from: private */
        public static final void a(b this$0) {
            kotlin.jvm.internal.t.i(this$0, "this$0");
            this$0.onUIReady();
        }

        @Override // com.ironsource.qg
        public void onClosed() {
            this.f12184b.onClosed();
        }

        @Override // com.ironsource.qg
        public void onUIReady() {
            if (ft.this.f12179f) {
                this.f12184b.onUIReady();
            } else {
                ft.this.f12174a.postDelayed(new Runnable() { // from class: com.ironsource.iw
                    @Override // java.lang.Runnable
                    public final void run() {
                        ft.b.a(this.f12671a);
                    }
                }, 500L);
            }
        }
    }

    public ft(@NotNull TestSuiteActivity activity, @NotNull qg uiLifeCycleListener, @NotNull JSONObject jsonData, @Nullable String str) {
        kotlin.jvm.internal.t.i(activity, "activity");
        kotlin.jvm.internal.t.i(uiLifeCycleListener, "uiLifeCycleListener");
        kotlin.jvm.internal.t.i(jsonData, "jsonData");
        this.f12174a = new Handler(activity.getMainLooper());
        this.f12175b = new WebView(activity);
        this.f12176c = new WebView(activity);
        this.f12177d = jsonData;
        this.f12180g = el.f11917p.d().r();
        a(activity, uiLifeCycleListener, str);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void e(ft this$0) {
        kotlin.jvm.internal.t.i(this$0, "this$0");
        this$0.f12179f = true;
    }

    @Nullable
    public final WebView d() {
        return this.f12175b;
    }

    @NotNull
    public final String a(@NotNull String methodName, @NotNull List<? extends Object> methodArgs) {
        kotlin.jvm.internal.t.i(methodName, "methodName");
        kotlin.jvm.internal.t.i(methodArgs, "methodArgs");
        String str = "TestSuiteJs." + methodName;
        String strB = et.f11994a.b(methodArgs);
        if (!(strB.length() > 0)) {
            return str + "()";
        }
        return str + '(' + strB + ')';
    }

    public final void b() {
        WebView webView = this.f12175b;
        if (webView != null) {
            webView.destroy();
            this.f12175b = null;
        }
    }

    @NotNull
    public final WebView c() {
        return this.f12176c;
    }

    public final void a() {
        b();
        this.f12176c.destroy();
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void a(ft this$0, String script) {
        kotlin.jvm.internal.t.i(this$0, "this$0");
        kotlin.jvm.internal.t.i(script, "$script");
        this$0.f12176c.loadUrl("javascript:" + script);
    }

    @SuppressLint({"SetJavaScriptEnabled", "AddJavascriptInterface"})
    private final void a(TestSuiteActivity testSuiteActivity, final qg qgVar, String str) {
        WebView webView = this.f12175b;
        WebSettings settings = webView != null ? webView.getSettings() : null;
        boolean z10 = true;
        if (settings != null) {
            settings.setJavaScriptEnabled(true);
        }
        this.f12176c.getSettings().setJavaScriptEnabled(true);
        List<? extends IronSource.AD_UNIT> listQ = kotlin.collections.v.q(IronSource.AD_UNIT.BANNER, IronSource.AD_UNIT.INTERSTITIAL);
        if (this.f12180g.a() != null) {
            List<IronSource.AD_UNIT> listP = com.ironsource.mediationsdk.p.m().p();
            kotlin.jvm.internal.t.h(listP, "getInstance().legacyInitializedAdUnits");
            Iterator<T> it = listP.iterator();
            while (it.hasNext()) {
                listQ.remove((IronSource.AD_UNIT) it.next());
            }
        } else {
            listQ.clear();
        }
        this.f12176c.addJavascriptInterface(new rs(new ns.a().a(listQ, testSuiteActivity, this.f12174a), new b(qgVar), this), "TestSuiteBridge");
        this.f12176c.setWebViewClient(new a(str));
        if (str != null && str.length() != 0) {
            z10 = false;
        }
        if (!z10) {
            this.f12176c.loadUrl(str);
        }
        WebView webView2 = this.f12175b;
        if (webView2 != null) {
            webView2.loadUrl("javascript:document.write(atob(''))");
        }
        this.f12174a.postDelayed(new Runnable() { // from class: com.ironsource.gw
            @Override // java.lang.Runnable
            public final void run() {
                ft.e(this.f12354a);
            }
        }, ExoPlayer.DEFAULT_DETACH_SURFACE_TIMEOUT_MS);
        this.f12174a.postDelayed(new Runnable() { // from class: com.ironsource.hw
            @Override // java.lang.Runnable
            public final void run() {
                ft.a(qgVar);
            }
        }, 5000L);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void a(qg uiLifeCycleListener) {
        kotlin.jvm.internal.t.i(uiLifeCycleListener, "$uiLifeCycleListener");
        uiLifeCycleListener.onUIReady();
    }

    public final void a(@NotNull final String script) {
        kotlin.jvm.internal.t.i(script, "script");
        this.f12174a.post(new Runnable() { // from class: com.ironsource.fw
            @Override // java.lang.Runnable
            public final void run() {
                ft.a(this.f12190a, script);
            }
        });
    }

    @Override // com.ironsource.ss
    public void a(@NotNull String callbackName, @NotNull IronSource.AD_UNIT adUnit, @NotNull List<? extends Object> args) {
        kotlin.jvm.internal.t.i(callbackName, "callbackName");
        kotlin.jvm.internal.t.i(adUnit, "adUnit");
        kotlin.jvm.internal.t.i(args, "args");
        if (this.f12178e) {
            String string = adUnit.toString();
            kotlin.jvm.internal.t.h(string, "adUnit.toString()");
            a(a("onCallbackReceived", kotlin.collections.v.n(callbackName, string, args)));
        }
    }
}
