    ...
    public ActivityTransitionResult(@NonNull @SafeParcelable.Param(id = 1) List<ActivityTransitionEvent> list) {
        ...
        if (!list.isEmpty()) {
            for (int i10 = 1; i10 < list.size(); i10++) {
                int i11 = i10 - 1;
                Preconditions.checkArgument(list.get(i10).getElapsedRealTimeNanos() >= list.get(i11).getElapsedRealTimeNanos(), "Transition out of order: ts1=%d, ts2=%d", Long.valueOf(list.get(i10).getElapsedRealTimeNanos()), Long.valueOf(list.get(i11).getElapsedRealTimeNanos()));
            }
        }
