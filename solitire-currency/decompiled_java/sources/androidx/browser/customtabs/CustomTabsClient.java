...
public class CustomTabsClient {
    ...
    private a.AbstractBinderC0011a createCallbackWrapper(@Nullable final CustomTabsCallback customTabsCallback) {
        return new a.AbstractBinderC0011a() { // from class: androidx.browser.customtabs.CustomTabsClient.2
    ...
    private CustomTabsSession newSessionInternal(@Nullable CustomTabsCallback customTabsCallback, @Nullable PendingIntent pendingIntent) {
        boolean zNewSession;
        a.AbstractBinderC0011a abstractBinderC0011aCreateCallbackWrapper = createCallbackWrapper(customTabsCallback);
        try {
            if (pendingIntent != null) {
                Bundle bundle = new Bundle();
                bundle.putParcelable(CustomTabsIntent.EXTRA_SESSION_ID, pendingIntent);
                zNewSession = this.mService.newSessionWithExtras(abstractBinderC0011aCreateCallbackWrapper, bundle);
            } else {
                zNewSession = this.mService.newSession(abstractBinderC0011aCreateCallbackWrapper);
            }
            if (zNewSession) {
                return new CustomTabsSession(this.mService, abstractBinderC0011aCreateCallbackWrapper, this.mServiceComponentName, pendingIntent);
            }
            return null;
