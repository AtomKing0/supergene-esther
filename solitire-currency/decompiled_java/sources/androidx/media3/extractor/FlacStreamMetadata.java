...
public final class FlacStreamMetadata {
    ...
    public long getApproxBytesPerFrame() {
        ...
        if (i10 > 0) {
            ...
            j10 = ((((i11 != this.maxBlockSizeSamples || i11 <= 0) ? 4096L : i11) * ((long) this.channels)) * ((long) this.bitsPerSample)) / 8;
            j11 = 64;
        }
        return j10 + j11;
    }
    ...
    public Format getFormat(byte[] bArr, @Nullable Metadata metadata) {
        ...
        return new Format.Builder().setSampleMimeType(MimeTypes.AUDIO_FLAC).setMaxInputSize(i10).setChannelCount(this.channels).setSampleRate(this.sampleRate).setPcmEncoding(Util.getPcmEncoding(this.bitsPerSample)).setInitializationData(Collections.singletonList(bArr)).setMetadata(getMetadataCopyWithAppendedEntriesFrom(metadata)).build();
    }
