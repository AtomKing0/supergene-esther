...
public final class FileTypes {
    ...
    public static String getFileExtensionFromUri(Uri uri) {
        ...
        if (iLastIndexOf2 >= 0) {
            path = path.substring(iLastIndexOf2 + 1);
        }
        return (path.isEmpty() || !Pattern.matches("[a-zA-Z_0-9\\.\\-\\(\\)\\%]+", path) || (iLastIndexOf = path.lastIndexOf(46)) < 0) ? "" : path.substring(iLastIndexOf + 1);
    }
