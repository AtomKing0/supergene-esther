...
## 시트 수정 마스터 워크플로우 ⚠️ 필수

모든 시트 데이터 변경은 반드시 아래 순서를 따른다. **순서 위반 시 재작업.**
...
- **라이브 시트**: `1oLbpsJjiDz0pdljaGBQznrSZcXA7aypFyJihnQ8_iiM`

## Google Sheet 수정 세부 규칙 (07 에이전트 작업 기준) ⚠️ 필수
...
## 코드리뷰 프로세스 (코딩 작업 후 필수)

코딩 작업(HTML 보고서 생성·수정, Python 스크립트 작성, Google Sheets API 코드 등)이 완료되면 **반드시** `10_code_review_agent`를 호출하여 코드리뷰를 수행한다.

### 자동 트리거 조건

아래 조건 중 하나라도 해당하면 코딩 완료 후 코드리뷰를 실행한다:

| 조건 | 예시 |
|------|------|
| HTML 파일 생성 또는 수정 | 보고서 신규 작성, 테마 토글 추가, 차트 수정 |
| Python 스크립트 생성 또는 수정 | 시뮬레이션 스크립트, 카탈로그 분석 |
| Google Sheets API 호출 코드 | 시트 수정, 동기화, 데이터 입력 |
| CSS/JS 로직 변경 | 스타일 변경, 인터랙션 추가 |

### 실행 방법

```
Task(10_code_review_agent, mode: CODE-REVIEW, scope: {변경된 파일 목록})
```

### 프로세스 흐름

```
코딩 작업 완료 → 10_code_review_agent 호출 → 리뷰 결과 확인
  ├─ PASS / PASS WITH MINOR → 작업 완료 보고
  └─ FAIL → CRITICAL/MAJOR 수정 → 재리뷰
```

### 면제 조건

- 단순 오탈자 수정 (1~2줄 텍스트만 변경)
- `sync_reports.sh` 실행만 수행한 경우
- CLAUDE.md / 에이전트 정의 파일만 수정한 경우
