...
| guide_type | focus_bubble(21건, 강제) / floating(7건, 비강제) |
...
- [ ] S1-1: 딤 없이 마스코트 Bubble 자동 등장 (카드 딜링 1.5초 후)
- [ ] S1-1: 매칭 가능 카드에 스파클 이펙트 표시
- [ ] S1-1: 3초 후 자동 소멸 또는 탭으로 소멸
- [ ] S1-2: 매칭 가능 카드 하이라이트 + Arrow(PULSE) 표시
- [ ] S1-2: **매칭 불가 카드 탭 → 카드 흔들기 0.3초 + 올바른 카드 깜빡임 3회**
- [ ] S1-2: 5초 무행동 → 매칭 가능 카드 힌트 깜빡임
- [ ] S1-2: 3회 매칭 후 자유 플레이 전환
- [ ] S1-4: 클리어 시 인게임 보상 수령 연출 (코인 파티클 30~50개 + 컨페티 + 마스코트 축하 + Collect 버튼)
- [ ] 클리어 시 골드 1,000G + α + 해머 표시
- [ ] **입장료 0 확인 (Lv 1~3)**
...
- [ ] S2-1: 이동 불가 시 드로우 덱 하이라이트 + 마스코트 Bubble
- [ ] S2-1: 드로우 덱 외 터치 → 피드백 (드로우 덱 Blink)
- [ ] **입력 제한 ON** (Lv 1~3 동일)
- [ ] Lv 2 클리어 후 **프리레벨 팝업 해금 (230005, val=2)**
- [ ] 프리레벨 팝업에서 참가비·보상·부스터 UI 안내 (난이도 표시 → 베팅 버튼 순차)

### Phase 2 (Lv 3~4)

- [ ] **Lv 3부터 입장료 차감 시작** (이전 레벨과 비교 확인)
- [ ] **Lv 3도 강제 클리어 스테이지** (입력 제한 ON, 무조건 클리어)
- [ ] Lv 3 클리어 → 상점 아이콘 focus_bubble (230004)
- [ ] Lv 4 클리어 → **방치골드→수령→출석체크 순차 강제 플로우** (3단계 모두 완료 필수)
  - [ ] 230006 방치골드 해금
...
- [ ] Lv 5 스트릭 미터 2연속 매칭 시 가이드 (230010) — focus_bubble, 인풋 일시정지
- [ ] Lv 5 클리어 후 리더보드 아이콘 터치 유도 (230011) — 마스코트 Bubble
...
- [ ] Lv 8 클리어 → 인게임 보상 수령 연출 + 마스코트 졸업 Bubble (`T_TUT_GRADUATION`)
...
- [ ] Lv 12 이벤트 강제 터치 유도 (230020) — 세션당 1회
...
- [ ] 동일 가이드 **중복 표시 없음** (1회 제한, 반복 가이드 제외)
...
### 4.1 Focus Bubble (강제, 21건)

| 항목 | 기대값 |
|------|--------|
| 딤 배경 | alpha 0.3(일반) / 0.4(로비). Phase 0 S1-1은 딤 없음 |
| 대상 강조 | 하이라이트 mask (스파클 이펙트 포함) |
| 말풍선 | 흰색 라운드 사각형(radius 12px, shadow) + 꼬리 (target 방향) |
...
| 말풍선 | 반투명, 캐릭터+말풍선 |
| 스킵 | 화면 아무 곳 탭 → 즉시 닫힘 |
| 대상 키 | 230001, 230002, 230013, 230015, 230016, 230027, 230028 |
...
| 위치 | 대상 부스터 버튼 상단 |
| 색상 | 초록(#4CAF50) 라운드 배지 |
| 적용 대상 | 와일드(Lv 7), 파이어웍스(Lv 8), 언두(Lv 8), 엑스트라 덱(상황), 골든 티켓(Lv 24) |
...
| 항목 | 기대값 |
|------|--------|
| 코인 파티클 | 30~50개, 상단→코인 카운터 비행 (2.0s, ease-out) |
| 컨페티 | 멀티컬러 5~8색, 1.5초 낙하 |
| 마스코트 | 상단 축하 포즈 (200×200px) |
| 보상 표시 | 화면 중앙 "+{amount} G ⚒×1" (24pt, Bold, 골드색) |
| Collect 버튼 | 초록 라운드 버튼, 탭 → 로비 전환 |
...
| 규칙 | 적용 구간 | 검증 방법 |
|------|----------|-----------|
| 입력 제한 (강제 클리어) | **Lv 1~3** | 매칭 불가 카드 탭 → 흔들기(0.3초)+깜빡임(0.5초×3) 확인. 무조건 클리어 가능 |
...
| Lv 3 강제 클리어 중 매칭 불가 | 드로우 덱으로 유도 (S2-1 패턴) | [ ] |
| Lv 8 클리어 후 졸업 Bubble | 마스코트 CELEBRATION + 3초 autoDismiss | [ ] |
| is_repeatable=TRUE 가이드 반복 확인 | 230026/230027/230028 조건 충족 시 매번 발동 | [ ] |

---

## 10. 무료 부스터 지급 검증

| 부스터 | 해금 시점 | guide_key | 지급 수량 | UI 표기 | guide_type |
|--------|----------|-----------|----------|---------|------------|
| 와일드 카드 | Lv 7 인게임 | 230013 | 3개 | FREE ×3 | floating (비강제) |
| 파이어웍스 | Lv 8 pre_level | 230014 | 3개 | FREE ×3 | focus_bubble |
| 언두 | Lv 8 인게임 | 230015 | 3개 | FREE ×3 | floating (비강제) |
| 엑스트라 덱 | 덱소진+매칭불가 | 230016 | 3개 | FREE ×3 | floating (비강제) |
| 골든 티켓 | Lv 24 | 230025 | 3개 | FREE ×3 | focus_bubble |

**검증 포인트**:
- [ ] 지급 후 잔여 수량 UI 반영
- [ ] 소진 후 유료 전환 (가격 표시 정상)
- [ ] 중복 지급 방지 (같은 부스터 2회 이상 무료 지급 안 됨)
- [ ] FreeTag 배지 표시 위치·색상 (#4CAF50)
...
| SCR | 화면 | 발생 시점 | 핵심 확인 |
|-----|------|-----------|----------|
| 01 | 인게임 Phase 0 가이드 | NRU Lv 1 | **딤 없이 마스코트 Bubble + 스파클** (S1-1), 딤 0.3 (S1-2~) |
| 02 | 로비 최초 진입 | Lv 1 클리어 후 | **3단계**: 마스코트 안내 → 건물 업그레이드 → 플레이 유도 |
| 03 | 도시 꾸미기 강제 | 망치 충족 시 | **매 구역 반복** (is_repeatable=TRUE) |
| 04 | Pre-Level 최초 | Lv 3 | 난이도 표시 → 베팅 버튼 순차 안내 |
| 05 | 기믹 첫 등장 | 기믹별 1회 | 인풋 일시정지 → Bubble → 탭 → 재개 |
| 06 | 스트릭 가이드 | Lv 5 첫 2콤보 | 스트릭 미터 하이라이트 + Arrow(PULSE) |
| 07 | 언두 튜토리얼 | Lv 8 | **floating + FREE×3** (비강제, 탭 스킵) |
| 08 | 엑스트라 덱 | 덱소진+매칭불가 | **상황 기반**, **floating + FREE×3** |
| 09 | 해금 팝업 | 각 해금 레벨 | 아이콘 80×80 + 타이틀 + 설명 + 확인 버튼 |
| 10 | 아이콘 터치 유도 | 상점/리더보드 등 | 딤 0.4 + 하이라이트 + Arrow(BOUNCE) + 마스코트 Bubble |
| 11 | 파이어웍스 Pre-Level | Lv 8 | 팝업 내 딤 + 프리부스터 슬롯 하이라이트 + FREE×3 |
| 12 | 타임아웃 힌트 | Lv 1~8 | 5초 무행동 → Blink. **Lv 9+ 비활성** |
| 13 | 클리어 연출 | Lv 1, Lv 8 | 코인 파티클 + 컨페티 + 마스코트 + Collect 버튼 |

---

## 12. Tutorial → Normal 전환 검증표

| 항목 | Lv 8 (Tutorial) | Lv 9 (Normal) | 검증 |
|------|----------------|---------------|------|
| difficulty_tier | Tutorial (tier_id=1) | Normal (tier_id=2) | [ ] |
| help_card_rate | 8000 (80%) | 5000 (50%) | [ ] |
| pity_trigger | 3회 | 5회 | [ ] |
| streak_sustain | 5회 | 4회 | [ ] |
| 맵 풀 | tutorial_guide 전용맵 | 일반 schedule | [ ] |
| DDA | 비활성 | Lv 11부터 활성 | [ ] |
| Lv 9~10 완충 | — | Normal 고정 (DDA 없음) | [ ] |
| 타임아웃 힌트 | 5초 무행동 Blink | 비활성 | [ ] |
